...
import pandas as pd
from palette_compression import painting
...
if __name__ == "__main__":
    ...
    if args.dbscan.lower() not in ["y", "yes", "n", "no"]:
        exit(
            f"{Fore.LIGHTRED_EX}ERROR: can't understand dbscan argument, available values: y, yes, n, no{Style.RESET_ALL}"
        )
    ...
    print(
        f"File size : {Fore.LIGHTRED_EX}{oldsize:,d}{Style.RESET_ALL} -> {Fore.LIGHTGREEN_EX}{newsize:,d}{Style.RESET_ALL} bytes"
    )
